...
//audiomusic play section at bottom
...
const LoggedInContainer = ({ children, currentActiveScreen }) => {
    ...
    //this useContext is used to get the currentaudiobook and setCurrentaudiobook from audiobookContext.Provider also it render once when we change the currentaudiobook
    const {
        currentAudiobook,
        setCurrentAudiobook,
        soundPlayed,
        ...
        isPaused,
        setIsPaused
    } = useContext(audiobookContext);

    //useEffect activates when currentaudiobook is changed also useeffect is resonsible for playing the music    
    //useEffect is used to reredner the component otherwise som changes that will happen in dependency it will use so we not use useEffect because when we move from one page to other the audiobook is rerendering or say start from begining so at background 2 audiobooks are playing (renderring)
    //so insted of this we use useLayoutEffect which is used to render the component before the useeffect so it will not rerender the component and the audiobook will not start from begining
    ...
    useLayoutEffect(() => {
        ...
        changeAudiobook(currentAudiobook.track)
    }, [currentAudiobook && currentAudiobook.track])
    ...
    const changeAudiobook = (audiobookSrc) => {
        if (soundPlayed) {
            soundPlayed.stop()
        }
        let sound = new Howl({
    ...
    const togglePlayPause = () => {
        if (isPaused) {
            playSound(currentAudiobook.track)
    ...
    return (
        <div className="h-full w-full  bg-app-black">
            ...
            <div className={`${currentAudiobook ? "h-9/10" : "h-full"} w-full flex`} >
                    ...
                    <div className="navbar h-1/10 w-full bg-black bg-opacity-30 flex items-center justify-end">
                        <div className="w-1/2 flex h-full">
                            ...
                            <div className="w-2/5 flex justify-around h-full items-center">
                                ...
                                <TextWithHover
                                    displayText={"Upload Audiobook"}
                                    targetLink={'/uploadaudiobook'} />
                    ...
                    <div className="w-1/3 flex items-center">
                        ...
                        <div className="pl-4">
                            ...
                            <div className="text-xm text-gray-400 hover:underline cursor-pointer">
                                {currentAudiobook.artist.firstName + " " + currentAudiobook.artist.lastName}
                            </div>
