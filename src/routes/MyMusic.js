...
const MyMusic = () => {
    ...
    useEffect(()=>{
        const getData = async()=>{
            const response = await makeAuthenticatedGETRequest("/audiobook/get/myaudiobooks")
    ...
    return (
        <LoggedInContainer currentActiveScreen={"myMusic"}>
            <div className="text-white text-2xl font-semibold pb-4 pl-2">My Audiobooks</div>
            <div className="space-y-3 overflow-auto">
                {audiobookData.map((item) => {
                    return <SingleAudiobookCard key={item.id} info={item} playSound={playSound} />
                })}
            </div>
        </LoggedInContainer>
    )
...
//                     <div className="space-y-3 overflow-auto">
//                        {audiobookData.map((item)=>{
//                             return <SingleaudiobookCard info={item} playSound={playSound}/>
//                        })}
//                     </div>
